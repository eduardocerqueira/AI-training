...
**Scheduled agents** (docs-bot, CVE scan, issue-bot, pr-bot, test-bot, experiment-agent): overview, cron schedules, and setup in [docs/agents.md](docs/agents.md). Bots that open PRs need **`BOT_GH_TOKEN`** (one PAT) so CI runs on those PRs. Issue bot (label `agent`) posts a plan and opens an implementation PR (v2).
